...
public class EmployeeRestService {
	...
	@PUT
	public JAXBElement<Employee> updateEmployee(JAXBElement<Employee> employee){
		Employee value = employee.getValue();
		value.setEname("乔布斯");
		value.getDepartment().setDeptname("apple");
		return new JAXBElement<Employee>(new QName("employee"), Employee.class, value);
	}
	
	@DELETE
	@Path("{id}")
	public JAXBElement<Employee> deleteEmployee(@PathParam("id")int id){
		Department department = new Department(id,"研发部");
		Employee employee = new Employee(id,"scott",department);
		return new JAXBElement<Employee>(new QName("employee"), Employee.class, employee);
	}
